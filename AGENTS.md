...
| 测试 | `test/smoke.js` `test/stages.test.js` `test/verdict.test.js` `test/journal.test.js` `test/diagnostic.test.js` `test/evidence.test.js` `test/metering.test.js` `test/gitignore.test.js` `test/product-scope.test.js` | 结构/描述符 smoke + 档位阶段集 + 验收结论 + journal 行为 + 重试诊断/交付判定（judgeDeliverable 信号分级）+ 验证证据块 + token 计量宿主适配（官方投影优先 + 多源回退/usage 双路径回退 + 熔断新增口径 freshTokensOf） + **收口提交面（自有日志 .gitignore 幂等合并 + pathspec 排除）** + 产品线装配（全局面板数据面：地址/白名单/过滤/摘要/空态） |
...
- **构建/验证**（插件目录下）：
  ...
  - `pnpm test` —— smoke + journal + verdict + stages + diagnostic + evidence + metering + gitignore + product-scope + prompt-contract + conformance（smoke 对 host 目录做源码断言：新增/移动函数后要同步指向；**改 prompt/注入必跑 L1 prompt-contract + L2 conformance；改计量/宿主适配必跑 metering；改收口提交面/日志忽略必跑 gitignore；改产品线装配/全局面板数据面必跑 product-scope**）
...
| 验证证据块 | dev/qaFix 回复末尾强制 `[Verification evidence]` 块（命令+退出码+断言计数+失败行引用，或显式 N/A）→ host 提取存证 `stage.verifyEvidence`（stageDetail 可见，可与 logs/ 命令输出对照）；policy 级——缺失记 warn 不中断。**日志布局（同 policy）**：每用途一个文件，**路径一律写全 `logs/teamflow/<runId>/...`**——`regression-<phase>.log`（套件输出，重跑**追加**带时间戳段，禁 `-run2`/`-nopipe`/`-shim` 变体）/ `scripts/`（一次性校验脚本）/ `captures.json`（命令载荷汇总）/ `probe/`（探针）；**项目根不得出现 `scripts/`、`probe/`**（实锤 tf-mtx6fi2a：未写全路径时模型在项目根建了这两个目录，6 个草稿被卷进交付提交）。实锤一次 run 51 份重复套件输出占日志 78% |
