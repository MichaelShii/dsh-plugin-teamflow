...
- ✅ **token 观测「观测→执行」闭环（2026-08-27 已实施）**：`host/core/guard.ts` 的 `warnOnce` 在第 3 次重复 read / 验证脚本重复执行时，除 journal warn 外同步 `run.inject` 轻提醒（不打断，下一轮 step 可见）——对治「17 条 warn 零削减」（实证 run tf-mtb58jz9）。只提醒不强制：重复读常是写断言的合理需求。
- 📝 **tech 分层注入已判定不做（2026-08-27，与 benchmark 复核同期）**：billed 省 2M 但真实成本仅 ~0.3M 等价（注入首轮 input 全价、后续 cacheRead 1/10）；风险 = dev 失去散文「为什么」→ QA 打回（v0.10.2 教训）+ 自由文本切段脆弱 + grep 逃生通道部分抵消收益；实证 run tf-mtb58jz9 在「蓝图 JSON + 全文注入」下 10 任务零打回。**触发条件：tech 产出 >15k token（~1500 行大需求）时重新评估**。
- 📝 **已判定假优化（勿再投入）**：子 agent 共享已读文件全文（注入=前缀全价重放，自读 cacheRead 便宜）、拆任务存在性预检（收益 0.2% < 成本）、护栏强制削减（替模型决策伤质量）、测试任务合并（合并后单 agent 上下文累积：估算 18.8M vs 分开 7.9M，分开便宜 58%）、tech 分层注入（见上条）。详见 `docs/benchmarks/hold-pipeline-vs-native.md` 复核节。
