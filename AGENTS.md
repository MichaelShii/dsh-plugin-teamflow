...
- 2026-08-19：阶段卡点击查看详情 —— 画布旁并排真实详情栏（不嵌套画布：悬停其内滚轮滚动正文、不触发画布缩放；面板撑开/收起时画布自动重新适配居中），官方口径 usage 全字段 + 阶段性产物全文 +「🎬 查看子代理会话」跳转（sessions.openSubagent，mode one-shot）；host 新增 stageDetail RPC（读 stage.output 全文）；终态 checkpoint 不再删 stage.output（磁盘+内存保留全文，供详情/断点续跑，smoke 断言同步）。
