...
| 测试 | `test/smoke.js` `test/locale.test.js` `test/stages.test.js` `test/verdict.test.js` `test/journal.test.js` `test/runlogs.test.js` `test/diagnostic.test.js` `test/evidence.test.js` `test/metering.test.js` `test/gitignore.test.js` `test/commit-path.test.js` `test/product-scope.test.js` | 结构/描述符 smoke + **语言层门禁（词典键唯一 / zh-en 同形 / en 无 CJK）** + 档位阶段集 + 验收结论 + journal 行为 + **日志生命周期（白名单过滤/归档搬运/合并/自愈清扫/活跃 run 豁免/K 次淘汰/永不抛 + store↔constants 路径同址）** + 重试诊断/交付判定（judgeDeliverable 信号分级）+ 验证证据块 + token 计量宿主适配（官方投影优先 + 多源回退/usage 双路径回退 + 熔断新增口径 freshTokensOf） + **收口提交面（自有日志 .gitignore 幂等合并 + 索引兜底参数形状）** + **收口提交链路的真 git 集成（负 pathspec 会 exit 1 的防回退锁 / 写规则→整树 add→索引兜底→commit 真跑通 / 「无事可做」判据）** + 产品线装配（全局面板数据面：地址/白名单/过滤/摘要/空态） |
