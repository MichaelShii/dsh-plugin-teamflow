...
- **构建/验证**（插件目录下）：
  ...
  - **发布**：`npm publish`（升 `package.json` version 后；`files` 白名单 = `lib`/`cordis.patch.yml`/`README.md`/`README.en.md`/`CHANGELOG.md`，`prepublishOnly` 自动 bundle+全套测试；包名无 scope 默认公开，registry 为 npmjs.org）。**发版 SOP**（2FA 人工节点、release note 必须由 `git log <上一tag>..HEAD` 生成而非凭记忆、已知坑清单）见 skill `release-npm-package`。**合入方式 = PR（2026-09-16 起确立）**：`release-vX.Y.Z` 分支 → 推分支 → PR → squash 合入 main → 删分支；v0.1.5–v0.1.7 与 v0.1.9 是历史直推，**v0.1.10 起不再直推 main**。**定稿顺序**：版本号 + CHANGELOG 日期 + `docs/releases/<v>.md` 一起定稿 → `npm publish`（人工节点）→ `git tag -a v<v>` 打在 main 的发布提交上并推送 → `gh release create --notes-file docs/releases/<v>.md`。**判定发布是否成功看写路径**：registry 读路径（`npm view` / packument / `/<pkg>/<v>`）可滞后数分钟，据此误判「没发成功」会导致重复引导发布；重发得到 403 `cannot publish over the previously published versions` 才是权威回执（幂等安全，无副作用）。
