...
- **脚本双态输出与测试方式（自 2026-09-26 / r11 起）**：`scripts/` 下的分析脚本默认保持人读输出，提供 `--json` 时 stdout **只**输出一个带 `schema` 版本号的结构化 JSON（零人读文案混入），人读态与机读态共用同一个 report 计算点（禁止各算一遍）；脚本测试一律用「import 导出纯函数 + 内存 fixture」，**不 spawn 子进程**（受限沙箱下管道 stdio 会 EPERM），新增测试须登记进 `package.json` 的 `test` 与 `prepublishOnly` 两条链。
