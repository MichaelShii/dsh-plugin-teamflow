...
- **按阶段下发 `reasoningEffort`（token 大杠杆）**：✅ v0.1.7 已落地两处机械阶段降档（patch 单点确认 + scaffold → `low`；重试回升 high；先经 `resolveModelInfo` 探测能力再下发），**A/B 已验证生效**——同需求两次 patch 档对比「单点确认」阶段：billed 197,071 → 103,788（**−47%**）、output 12,649 → 3,421（**−73%**）、cacheRead −50%、耗时 68.7s → 24.6s；两次确认单都完整（判定 + 实测现状核对 + 文件清单），dev 自测通过、零重试零人工介入。⚠️ 口径诚实说明：**n=1/臂**且 calls 6→5，样本差异与 effort 混在一起；output 的 −73% 与「推理 token 计入 output 且每回合回传」的机制方向一致，但不据此宣称精确收益。**剩余**：① 若要把数字做稳，同需求再跑 2 轮取均值；② 是否把 `dev` 的机械改动也纳入降档（当前保守不动——dev 在候选轮 cacheRead 302k 是最大头，但它是判据型阶段）。
