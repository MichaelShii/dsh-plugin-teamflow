...
- 🔜 **护栏事件读取迁移（dsh 0.1.5-rc.2 弃用同步读取器）**：`guard.eventsOf` 仍走 `snapshotEvents()/ownEvents()`——宿主 2026-09-09 起「存量可留、新调用禁止」，方向是不再常驻完整事件序列，届时该回退会失明（表现为护栏判「挂死/空转」的依据消失，`isAgentBusy` 活动守卫仍在）。计量已在 v0.1.7 迁到官方 Session 投影；护栏要的是近期事件**内容**（复读/挂死判据），投影不提供——替代路径二选一：① 判据改为 `session.deriveMessages()`（未弃用）+ 增量处理送达事件；② 自注册投影（`ctx.sessionProjections.register`）维护近期事件环形状态。**动手前先定等价判据**（判据漂移直接影响误杀率，参考 tf-mte906e9 退化误判与 json-parse r1 挂死误杀两次实锤）。
