...
**一句话**：在会话里说一句需求，TeamFlow 拉起一支子代理团队跑完 `需求 → PRD → （UI/UX 设计）→ （脚手架）→ 技术方案 → 并行开发 → QA → 验收`，每一步产物落盘成文件，token 按宿主官方口径计量，中途崩了能从断点续跑。

它不是「一句话生成一个 App」那种玩具——**重心在工程纪律**：产物即文件、验收结论有契约、失败要显式暴露、崩了能续跑、花的每一分 token 都对得上官方账单。
