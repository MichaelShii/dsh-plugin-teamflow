...
ok(/stage\.guardOutcome \|\| 'degenerated'/.test(hostSrc), 'runner：复读=degenerated 可重试；挂死/空转=stalled 走预算门转人工')
ok(/const guardedRetry = !!\(lastStage && lastStage\.outcome === 'degenerated'\)/.test(hostSrc), 'runner：仅复读中止豁免预算门，允许一次干净重试')
...
ok(/mkdirSync\(abs, \{ recursive: true \}\)/.test(pipelineSrc) && /meta\.json/.test(pipelineSrc), 'pipeline：建夹 + meta.json（创建与终态回写）')
...
ok(/基线依赖|取代：/.test(promptsSrc) && /AC 编号】本夹内从 AC-1/.test(promptsSrc), 'prompts：局部 AC 编号 + 基线依赖/取代声明')
ok(/slug/.test(readFileSync(join(here, '../host/core/triage.ts'), 'utf8')) && /TRIAGE_PROMPT/.test(promptsSrc) && /主题词/.test(promptsSrc), 'triage：slug 输出字段（受控命名来源）')
