...
export const ARTIFACT_CONTRACTS: Record<ArtifactKind, ArtifactContractItem[]> = {
  ...
  'plugin-host': [
    ...
    { requirement: '能被 profile 真实装入（**以本机探测到的安装入口为准，不得写死 profile 名/路径/命令**）', criteria: '用 host 在 PRD 里注入的「本机环境」那段给出的命令（CLI 形式，或 `dsh` 不在 PATH 时的等价手动步骤：在探测到的 profile 目录 `pnpm add` + 该包进 `dsh.profile.bundles`）；装后 profile 的 `dependencies` 出现该包、`bundles` 含包名。**探测失败时改为问用户 profile 位置，不许编路径**', onlyWhenInstallable: true },
    { requirement: '装载后宿主真实加载该插件（端到端判据）', criteria: '重启宿主后启动日志/插件列表出现该插件；仅有源码文件不算', onlyWhenInstallable: true },
    { requirement: '**安装必须带回滚**（实锤 dddd：装上后宿主起不来，只能另开 agent 手术卸载）', criteria: '安装前先写明卸载步骤（反向操作：`pnpm remove <包名>` / `dsh plugin remove <name>`，并确认该包已从 `dsh.profile.bundles` 摘除；包名从产物 package.json 读，不写死），装后验证失败 → 立即执行卸载恢复', onlyWhenInstallable: true },
  ],
  'plugin-client': [
    ...
    { requirement: '**安装由主 agent 执行（子代理装不了），PRD 必须给出本机可照做的步骤**', criteria: '流水线子代理权限被钉死在工作区（会话原文 `permission scope was fixed ... cannot be widened`）→ 写不了 profile；**主 agent 能**（profile 写入被拒时宿主给 `escalation available`，`approval/policy: ask` 下经用户批准即可）。故 PRD 要写成「给主 agent 的安装步骤」：本机命令（取自 host 注入的本机环境）+ 重启宿主 + GUI 可见性检查三步；**不得因为子代理跑不了就默认通过**', onlyWhenInstallable: true },
  ],
  'plugin-full': [
    ...
    { requirement: '能被 profile 真实装入且被宿主加载（端到端；**以本机探测到的安装入口为准，不得写死 profile 名/路径/命令**）', criteria: '用 host 注入的「本机环境」给出的命令（CLI 形式，或 `dsh` 不在 PATH 时的等价手动步骤：探测到的 profile 目录内 `pnpm add` + 该包进 `dsh.profile.bundles`）+ 重启宿主后启动日志出现该插件；探测失败时问用户，不许编路径', onlyWhenInstallable: true },
    { requirement: '**安装必须带回滚**（实锤 dddd：装上后宿主起不来，只能另开 agent 手术卸载才能救回宿主）', criteria: '安装前先写明卸载步骤（反向操作：`pnpm remove <包名>` / `dsh plugin remove <name>`，并确认该包已从 `dsh.profile.bundles` 摘除；包名从产物 package.json 读，不写死），装后验证失败 → 立即执行卸载恢复', onlyWhenInstallable: true },
    { requirement: '**安装由主 agent 执行（子代理装不了），PRD 必须给出本机可照做的步骤**', criteria: '流水线子代理权限被钉死在工作区（会话原文 `permission scope was fixed ... cannot be widened`）→ 写不了 profile；**主 agent 能**（被拒后宿主给 `escalation available`，`approval/policy: ask` 下经用户批准即可）。PRD 写成「给主 agent 的安装步骤」：本机命令 + 重启宿主 + GUI 可见性检查三步；**不得因为子代理跑不了就默认通过**（实锤：AC-7 被列为"人工补测"后无人真跑，用户装完才发现问题）', onlyWhenInstallable: true },
  ],
  cli: [
