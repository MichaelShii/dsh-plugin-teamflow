...
- **🏭 团队工作台（双入口）**：
  ...
  - **全局面板**（v0.1.8）：侧边栏图标 → 中央主区整块切换为**产品线视角**——左栏产品线列表（`$DSH_HOME/teamflow/<key>`，含 run 计数/活跃数/最近需求与结论），右栏该产品线的 run 列表 + backlog 分组（不依附会话，跨会话可用）。点 run 在**右侧栏**打开完整详情（阶段/尝试/验证证据/产出/日志），可与任务夹产物并排看；右侧栏不可用时自动降级为面板内联
